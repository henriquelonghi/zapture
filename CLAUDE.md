...
**Visual identity (2026-07-23):** one shared identity across the whole site — see `Identidade visual Zapture.zip` at the repo root for the source kit. Tokens live in `index.css` (`:root`): `--ink`/`--paper`/`--mist`/`--border` (neutrals), `--zapture-green`/`--zapture-green-dark`/`--zapture-green-100`/`--zapture-blue`/`--zapture-gradient` (brand), aliased to the generic `--accent`/`--accent-dark`/`--accent-100` names used throughout components. Space Grotesk (headings, the logo wordmark, "destaque" numbers like `.metric-value`/`.report-preview-value`/`.pricing-value strong`) + Inter (body/interface) loaded via Google Fonts in `index.html`. `Logo.tsx` renders the mark as CSS divs (gradient squircle + 3 white bars, sized proportionally), not an SVG — adjust the ratios there, not `landing.css`, if the mark ever needs to change shape. `landing.css` (imported only by `Landing.tsx`) and `App.css`/`index.css` (dashboard) stay as separate files purely to keep the logged-in app's CSS payload from carrying marketing-page-only rules (`.hero`, `.pricing-card`, etc.) — that split is about payload size, not about the two surfaces having different colors/fonts anymore; they now pull from the same root tokens.
